...
class Tokenizer implements Iterator<Token> {
    ...
    private void lexOneToken() throws IOException, EOFException {
        ...
        default:
            if (Character.isLetter(c))
            	...
            	System.out.println("unexpected in tokenizer");
            	unexpected();
            }
        }
    }
    ...
    /**
     * Check if the next token read from the Tokenizer is the given TokenType
     * @param tt the given TokenType
     * @return true if the token is of the given type
     */
    public boolean peekType(TokenType tt){
    	return peek().getType().equals(tt);
    }
    
    /**
     * check if this slash is a DIV or a comment and set the next token
     * @throws IOException
     * @throws EOFException
     */
    private void lexSlash() throws IOException, EOFException {
		...
		int[] cs=peekReader(1);
		if (cs[0] == -1) {
            consume('/'); // Slash already in the system
            setNextTokenAndReset(DIV);
        }
		else{
			if((char) cs[0]=='/'){
    ...
    /**
     * ignore token until it turns into a new line, set token to the first char that is not white space
     * @throws IOException
     * @throws EOFException
     */
	private void setTokenToNextLine() throws IOException, EOFException {
		buf.setLength(0);
		char c=getNextCharAndAppend();	
		while(c!='\n') {
			c=getNextCharAndAppend();
			buf.setLength(0);
		}
